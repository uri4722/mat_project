...
function MemorialProfile({passedAwayArray}) {
    console.log(passedAwayArray);
    const { id } = useParams();
    const [profile, setProfile] = useState({})
    useEffect(() => {

        if (passedAwayArray.length === 0){
            console.log("no passedAwayArray");
            return;
        } 
        // Your code here
        const res = passedAwayArray.find(p => p.passed_away_id === Number(id));
        console.log(res);
        res.mishnaiot = [
            [
